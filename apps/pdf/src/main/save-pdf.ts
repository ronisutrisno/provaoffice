...
const STATIC_FORM_FILLS_KEY = PDFName.of('PROVAOfficeStaticFormFills')
...
function setVisualSignatureMetadata(annot: PDFDict, fieldName: string | undefined): void {
  if (!fieldName) return
  annot.set(PDFName.of('PROVAOfficeFormField'), PDFHexString.fromText(fieldName))
  annot.set(
    PDFName.of('Contents'),
...
async function addImageStamp(
  ...
  annot.set(PDFName.of('T'), PDFHexString.fromText('PROVAOffice'))
...
function addDrawing(
  ...
  if (d.kind === 'note') {
    ...
    annot.set(PDFName.of('T'), PDFHexString.fromText(d.author || 'PROVAOffice'))
