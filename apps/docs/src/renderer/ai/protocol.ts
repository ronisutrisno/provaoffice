...
export const COMMANDS_GUIDE = [
  ...
  '- When a generated document has BOTH a cover page and a table of contents, they must be on SEPARATE pages: cover on page 1, TOC starting on page 2, body content starting on page 3. Apply {"pageBreakBefore":true} (via updateParagraphStyle, fields ["pageBreakBefore"]) to the first TOC block AND to the first body block after the TOC — never let the cover and TOC share one page',
  '',
  'Discipline:',
...
export const AGENT_SYSTEM_PROMPT = [
  ...
  '- Web search budget: each search returns rich, dense results. Do AT MOST 1-2 web searches per task — combine all your questions into ONE broad query, then write the full document from those results. Do not search repeatedly for each section.',
  '- Final summary in chat: do NOT use markdown tables (they render poorly in the side panel); use plain sentences or bullet lists instead.',
  '',
  '# HTML fragment rules',
  HTML_RULES,
  '',
