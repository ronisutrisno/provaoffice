...
const localAlias = {
  '@prova/docx-engine': resolve(__dirname, '../../packages/docx-engine/src/index.ts'),
}
...
export default defineConfig({
  ...
  // @prova/* deps ship as raw TS source with extensionless imports, so they
  // must be bundled — externalizing them yields ERR_MODULE_NOT_FOUND under Node
  // (same setup as apps/slides).
  main: {
    plugins: [
      externalizeDepsPlugin({ exclude: ['@prova/electron-utils', '@prova/font-metrics'] }),
    ],
  ...
  renderer: {
    ...
    resolve: { alias: localAlias },
    server: {
      // Overridable so multiple PROVAOffice dev instances can coexist (default 5173).
