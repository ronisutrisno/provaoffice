...
import { createI18n, getUiLang, type Lang, normalizeLang, setUiLang } from '@prova/i18n'
...
/** office/pdf formats extract text via @prova/file-parse; images skip text
...
/** Attachment text extraction via @prova/file-parse (docx/pdf/pptx/xlsx/plain text) */
...
export function registerSheetsAiIpc(): void {
  ...
  // PROVA-AI account (gsk login state): the auth source for AI features; the
  ...
  ipcMain.handle(IPC_CHANNELS.aiStream, async (event, input: unknown) => {
    ...
    const provider = request.settings.provider as AiProviderId
    let config = request.settings.providers[provider]
    // PROVA-AI's key never enters the settings file; it is read from the gsk
