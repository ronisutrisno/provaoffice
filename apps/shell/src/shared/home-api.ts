...
/** UI language; kept self-contained here (mirrors Lang in @prova/i18n) */
...
export interface HomeApi {
  ...
  newFlow(opts?: { projectId?: string }): Promise<void>
  ...
  /** PROVA-AI account status (gsk login state; to be upgraded to a signup/account system later) */
  accountStatus(): Promise<AccountStatus>
  /** start PROVA-AI login (opens the browser; accountStatus flips to logged-in on completion); returns whether the launch succeeded */
  ...
  /** effective default save folder for new/untitled files (configured in userData/app-settings.json, falls back to <Documents>/PROVAOffice) */
  ...
  /** open the PROVA-AI credit-usage page in the default browser */
  ...
  /** sync the full list from PROVA-AI and return it (1 request when nothing changed); null when the sync failed */
...
/** a PROVA-AI web project shown in the home cloud section */
...
export interface AccountStatus {
  ...
  loggedIn: boolean
  email?: string
  /** remaining PROVA-AI credits (absent when the balance query failed) */
